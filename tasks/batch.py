...
from .helpers import emit_run_update

logger = logging.getLogger(__name__)

@celery.task(bind=True, acks_late=True, base=ContextTask, name='tasks.handle_batch_completion')
@with_session
def handle_batch_completion(self, results, test_run_id, num_cases_in_batch):
    ...
    try:
        ...
        updated = db.session.query(TestRun).filter_by(id=test_run_id).update(
        ...
        if updated == 0:
            logger.warning(f"BatchCallback {self.request.id}: No rows updated for TR:{test_run_id}.")
        ...
        if test_run.progress_current > test_run.progress_total:
            logger.warning(f"BatchCallback {self.request.id}: Capping over-increment for TR:{test_run_id}. "
        ...
        logger.info(f"BatchCallback {self.request.id}: Updated TR:{test_run_id} to "
        ...
        logger.error(f"BatchCallback {self.request.id}: Error for TR:{test_run_id}: {e}", exc_info=True)
